...
// ============================================
// 📦 PHẦN 7.5: CHỨC NĂNG CHỈNH SỬA VÀ XÓA
// ============================================

function editOrder(orderId) {
  console.log(`✏️ Editing order ID: ${orderId}`);
  
  // Tạo modal chỉnh sửa đơn hàng - hiển thị modal chi tiết đầu tiên
  viewOrderDetail(orderId);
  
  // Sau đó tạo nút Edit trong modal
  setTimeout(() => {
    ...
    // Thêm vào footer của modal chi tiết
    const footer = document.querySelector(".modal-footer");
    if (footer && !document.getElementById("editOrderBtn")) {
      ...
      <div class="modal" id="editOrderModal">
        <div class="modal-content" style="max-width: 700px;">
          <div class="modal-header">
            <h3 class="modal-title">Chỉnh Sửa Đơn Hàng</h3>
          ...
          <div class="modal-body">
            <form id="editOrderForm">
              <div class="filter-group">
                ...
                <input type="text" class="filter-input" value="${order.invoice_code || order.code}" disabled>
              </div>
              
              <div class="filter-group">
                <label class="filter-label">Khách hàng:</label>
                <select class="filter-select" id="editCustomerSelect">
                  <option value="${order.customer?.id || ''}" selected>${order.customer?.name || 'Chọn khách hàng'}</option>
                </select>
              </div>
              
              <div class="filter-group">
                <label class="filter-label">Ghi chú:</label>
                <textarea class="filter-input" id="editOrderNotes" rows="4">${order.notes || ''}</textarea>
              </div>
              
              <div class="filter-group">
                <label class="filter-label">Địa chỉ giao hàng:</label>
                <input type="text" class="filter-input" id="editOrderAddress" value="${order.shipping_address || order.address || ''}">
              </div>
            </form>
          </div>
...
async function saveEditOrder(orderId) {
  try {
    ...
    // Làm mới danh sách
    await renderOrdersTable();
    showToast("Thành công", "Đã cập nhật đơn hàng", "success");
    
  } catch (error) {
    console.error("❌ Error saving order:", error);
    showToast("Lỗi", `Không thể cập nhật: ${error.message}`, "error");
  }
}
...
async function deleteOrder(orderId) {
  ...
  try {
    ...
    await renderOrdersTable();
    await loadOrderStats();
    showToast("Thành công", "Đã xóa đơn hàng", "success");
    
  } catch (error) {
    console.error("❌ Error deleting order:", error);
    showToast("Lỗi", `Không thể xóa: ${error.message}`, "error");
  }
}
...
// ============================================
// 📦 PHẦN 12.5: CHỨC NĂNG TẠOMỚI, IN, XUẤT
// ============================================

function showCreateOrderModal() {
  console.log("➕ Showing create order modal");
      ...
      <div class="modal-content" style="max-width: 700px;">
        ...
        <div class="modal-body">
          <form id="createOrderForm">
            <div class="filter-group">
              <label class="filter-label">Khách Hàng <span style="color: red;">*</span></label>
              <select class="filter-select" id="newOrderCustomer" required>
                <option value="">-- Chọn khách hàng --</option>
              </select>
            </div>
            
            <div class="filter-group">
              <label class="filter-label">Nhân Viên <span style="color: red;">*</span></label>
              <select class="filter-select" id="newOrderEmployee" required>
                <option value="">-- Chọn nhân viên --</option>
              </select>
            </div>
            
            <div class="filter-group">
              <label class="filter-label">Phương Thức Thanh Toán <span style="color: red;">*</span></label>
              <select class="filter-select" id="newOrderPayment" required>
            ...
            <div class="filter-group">
              <label class="filter-label">Địa Chỉ Giao Hàng</label>
              <input type="text" class="filter-input" id="newOrderAddress" placeholder="Nhập địa chỉ giao hàng">
            </div>
            
            <div class="filter-group">
              <label class="filter-label">Ghi Chú</label>
              <textarea class="filter-input" id="newOrderNotes" rows="3" placeholder="Nhập ghi chú (tùy chọn)"></textarea>
            </div>
          </form>
        </div>
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" id="saveCreateBtn">
...
async function loadCreateOrderData() {
  try {
    const [customersResp, employeesResp] = await Promise.all([
      ...
      orderAPI.getEmployees()
    ]);
    
    let customers = customersResp.data || customersResp;
    let employees = employeesResp.data || employeesResp;
    
    if (!Array.isArray(customers)) customers = [];
    if (!Array.isArray(employees)) employees = [];
    
    const customerSelect = document.getElementById("newOrderCustomer");
    const employeeSelect = document.getElementById("newOrderEmployee");
    
    if (customerSelect) {
      customers.forEach(customer => {
        const option = document.createElement("option");
        option.value = customer.id;
        option.textContent = customer.name || customer.full_name || "Khách hàng";
    ...
    if (employeeSelect) {
      employees.forEach(employee => {
        ...
        option.textContent = employee.full_name || employee.name || "Nhân viên";
...
async function saveNewOrder() {
  ...
  const employeeId = document.getElementById("newOrderEmployee")?.value;
  ...
  if (!customerId || !employeeId || !paymentMethod) {
  ...
  try {
    ...
    await renderOrdersTable();
    await loadOrderStats();
    showToast("Thành công", "Đã tạo đơn hàng mới", "success");
    
  } catch (error) {
    console.error("❌ Error creating order:", error);
    showToast("Lỗi", `Không thể tạo đơn hàng: ${error.message}`, "error");
  }
}

function printInvoice(orderId) {
  console.log(`🖨️ Printing invoice for order ID: ${orderId}`);
  
  const modal = document.getElementById("orderDetailModal");
  if (!modal) {
    showToast("Lỗi", "Vui lòng mở chi tiết đơn hàng trước", "error");
    return;
  }
  
  const printContent = modal.querySelector(".order-detail-container").innerHTML;
  ...
  printWindow.document.write(`
    ...
    <head>
      ...
      <style>
        ...
        .order-detail-section h4 { border-bottom: 2px solid #333; padding-bottom: 10px; margin-bottom: 15px; }
    ...
    <body>
      <h1 style="text-align: center; margin-bottom: 30px;">HOÁ ĐƠN BÁN HÀNG</h1>
      ${printContent}
      <script>
        window.print();
        window.close();
  ...
  showToast("Thành công", "Đã mở cửa sổ in", "success");
}
...
function exportToExcel() {
  ...
  if (filteredOrders.length === 0) {
    showToast("Cảnh báo", "Không có đơn hàng để xuất", "warning");
    return;
  }
  ...
  try {
    ...
    const rows = filteredOrders.map(order => [
      order.invoice_code || order.code || order.id,
      formatDate(order.created_at || order.order_date),
      order.customer?.name || order.customer?.full_name || "N/A",
      order.customer?.phone || "N/A",
      ((order.invoice_details || order.items) || []).map(item => item.product?.product_name || item.product_name || "Sản phẩm").join("; "),
    ...
    link.setAttribute("download", `don-hang-${new Date().getTime()}.csv`);
    ...
    showToast("Thành công", `Đã xuất ${filteredOrders.length} đơn hàng`, "success");
