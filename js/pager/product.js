...
function updatePaginationInfo(paginationData) {
  ...
  const totalPages =
    Math.ceil(totalItems / itemsPerPage) || paginationData.last_page || 1;
...
async function loadStats() {
  try {
    ...
    if (response.success && response.data) {
      ...
      const lowStockElement = document.querySelector(
      ...
      if (outOfStockElement)
        outOfStockElement.textContent = stats.out_of_stock || 0;
    }
  } catch (error) {
...
function updateCategoryFilter(categories) {
  ...
  categories.forEach((category) => {
    const option = document.createElement("option");
    const categoryValue =
      typeof category === "object" ? category.value || category.id : category;
    const categoryLabel =
